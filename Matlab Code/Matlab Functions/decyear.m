...
%   DY = DECYEAR( T ) converts one or more datetime arrays T into decimal
%   year DY.  Input T can be an M-by-1 or and 1-by-M datetime array.
%   DECYEAR returns a column or row vector of M decimal years. 
%
...
%   Examples:
%
%   Calculate decimal year for February 4, 2016:
...
if nargin == 2 % Entry is char array with date and format
    % formatIn not consistent between datenum and datetime
    dateString = varargin{1};
    formatIn = varargin{2};
    dateIn = datetime( datenum(dateString, formatIn), 'convertFrom', 'datenum' );
else
    % Non-negative year(s) caught below, but other values allowed negative
    % and decimal values to preserve user's experience    
    if isa(varargin{1},"datetime")
        dateIn = varargin{:};
    else
        dateIn = datetime( datenum(varargin{:}), 'convertFrom', 'datenum' );
    end
end
...
ndays(leapyear(yearIn)) = 366;

firstDayofYear = datetime(yearIn, ones(size(yearIn)), ones(size(yearIn)));

dayofyear = dateIn - firstDayofYear;

% Calculate the decimal year
dyear = yearIn + dayofyear./hours(ndays*24);
